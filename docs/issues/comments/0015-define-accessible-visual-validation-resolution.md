...
| Profile management | Open, Reset to Generic, Delete | `Open Site Profile: <main domain>`, `Reset Site Profile <main domain> to Generic Profile`, and `Delete Site Profile: <main domain>`. Reset and delete require named confirmations describing all affected bindings and the Generic Profile fallback. |
...
| Input & timing | Hold Threshold, Press Sequence Window, Debounce | Each spin control includes its unit in the accessible name and announces its current value. Do not retain the obsolete `Double-click window` terminology. |
| Advanced | Theme preference | `Theme preference` with `System`, `Light`, and `Dark` options. The selected option is exposed as selected. |
| Advanced | Browser and Chromium Profile setup | Labels identify `Browser`, `Chromium Profile`, and `CDP port` distinctly. |
...
- The tray icon has a dedicated monochrome **Gesture Wheel** variant. At 16 px it uses a one-color foreground and transparent or system background, retains the wheel/notched-input silhouette, and contains no text, color-dependent meaning, caret, controller, weapon, or media-play glyph. The same concept is used for the application icon and runtime fallback.

## DPI And Text-Scaling Validation

Validate the Control Deck at 100%, 150%, 200%, and 300% Windows text scaling, plus 100%, 150%, 200%, and 300% display scaling where supported. Validate both a typical desktop window and the narrowest supported Control Deck width.
