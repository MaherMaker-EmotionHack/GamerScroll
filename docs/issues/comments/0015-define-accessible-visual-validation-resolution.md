...
3. Screenshots of loading, empty setup, ready, disabled, Browser unavailable (CDP), and missing-Pinned-Tab states that show an icon, plain-language label, and contextual recovery action without relying on color. Browser unavailable says that Gestures cannot be delivered and offers only `Reconnect browser`; the missing-Pinned-Tab state explicitly states that control returned to the active browser tab and offers only `Pin Current Tab`.
