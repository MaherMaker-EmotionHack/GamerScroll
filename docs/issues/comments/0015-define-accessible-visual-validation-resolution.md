...
- When listening is disabled, readiness announces `Listening disabled: Gestures will not be sent` and offers `Enable GamerScroll` as its one recovery action. Activating it returns focus to the readiness status and announces `Ready` or the applicable degraded state.
...
- The Gesture map and Generic and Site Profile editors are generated from the developer-owned Gesture Registry. They show Short Press, Double Press, Triple Press, and Long Hold, plus every future registered Gesture, as one binding row each; a newly introduced row is `Unassigned` until configured.
